...
class GPIOTestController:
    ...
    def on_start_glove_listen(self):
        ...
        if not self.device_connected:
            print("⚠️  [模拟] 设备未连接，继续以打印方式模拟手套监听")
        
        with self.glove_lock:
            if self.glove_listening:
                print("⚠️  [模拟] 手套监听已在运行中")
                return
    ...
    def start_glove_listening(self):
        ...
        print("    [模拟] 注册手套数据回调函数...")
        time.sleep(0.3)
        print("✅ [模拟] 手套UDP接收器已启动")
        
        # 启动模拟手套数据生成线程
        glove_thread = threading.Thread(target=self._simulate_glove_data, daemon=True)
        glove_thread.start()
    ...
    def stop_glove_listening(self):
        ...
        print("    [模拟] 关闭UDP套接字...")
        time.sleep(0.3)
        print("✅ [模拟] 手套UDP接收器已停止")
        
        # 恢复状态指示
        if not self.motion_running:
            self.gpio.set_rgb_color(0, 255, 0)  # 绿色：待命
    
    def _simulate_glove_data(self):
        """模拟手套数据生成（模拟）"""
        try:
            count = 0
            while True:
                ...
                count += 1
                print(f"\n📡 [模拟] 收到手套数据 #{count}")
                print("    [模拟] 设备名称: teleop_AHST006248D")
                print("    [模拟] 右手校准状态: 未校准")
                print("    [模拟] 左手校准状态: 未校准")
                print("    [模拟] 右手角度: -10.00, 0.00, 0.00, -0.20, 0.00, 0.00")
                print("    [模拟] 左手角度: -10.00, -3.40, -3.70, -3.90, -4.00, -4.00")
                print("    " + "-"*50)
    ...
    def print_status(self):
        ...
        print("手套监听状态: {}".format("🎧 运行中" if self.glove_listening else "⏸️  已停止"))
        print("="*60)
    ...
    def run(self):
        ...
        print("  GPIO {}: 开始手套监听".format(GPIO_PINS.START_GLOVE_LISTEN))
        print("\n输出引脚:")
