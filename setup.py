...
def step4_extract_demo(stdscr):
    ...
    if subprocess.run(["which", "7z"], capture_output=True).returncode == 0:
        subprocess.run(["7z", "x", demo_file, f"-o{target_dir}", "-y"], stdout=subprocess.DEVNULL, check=True)
    elif subprocess.run(["which", "7za"], capture_output=True).returncode == 0:
        subprocess.run(["7za", "x", demo_file, f"-o{target_dir}", "-y"], stdout=subprocess.DEVNULL, check=True)
    else:
...
def step5_select_config(stdscr):
    ...
    config_display = {
        "config_DH116_CANFD_exhibit.py": "DH116 - CANFD - 展示",
        "config_DH116_ECAT_exhibit.py": "DH116 - ECAT - 展示",
        "config_DH116_RS485_exhibit.py": "DH116 - RS485 - 展示",
        ...
        "config_DH116S_CANFD_exhibit.py": "DH116S - CANFD - 展示",
        ...
        "config_DH116S_CANFD_grasp_aging.py": "DH116S - CANFD - 抓握老化测试",
    ...
    def sort_key(filepath):
        ...
        if basename.startswith("config_DH116S"):
            return (1, basename)
        if basename.startswith("config_DH116"):
            return (0, basename)
        if basename.startswith("config_Module"):
            return (2, basename)
        return (3, basename)
    ...
    for file_path in config_files:
        ...
        config_names.append(config_display.get(basename, basename))
